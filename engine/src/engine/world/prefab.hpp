...
namespace kokoro
{
	namespace world
	{
		namespace prefab
		{
			...
			[[nodiscard]] core::cuuid		create(cview<cworld> w, std::string_view filepath, std::optional<sconfig> cfg = std::nullopt);
			[[nodiscard]] core::cuuid		create(cview<cworld> w, const sscene& data, std::optional<sconfig> cfg = std::nullopt);
